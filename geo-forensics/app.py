"""דשבורד GeoForensics — עיצוב Clinical (Claude Design).

הגרסה הקנונית. הגרסה המקורית שמורה ב-app_legacy.py.
...
<b>חולשות:</b> מניח קשרים ליניאריים בלבד; רגיש לקנה מידה של המשתנים; כשרכיבים מסבירים אחוז נמוך — הגרף מטעה; קשה לפרש loadings מעורבי סימן.
    ...
    # --- MDS (full width) ---
    st.markdown('<div style="font-family:\'Frank Ruhl Libre\',serif; font-size:15px; font-weight:600; margin:14px 0 8px; direction:rtl;">MDS — מיפוי מרחק כימי</div>', unsafe_allow_html=True)
    with st.expander("מתודולוגיה", expanded=False):
        st.markdown("""<div class="v2-method">
    ...
    with st.container():
        if not sim_matrix_nonzero.empty and len(sim_matrix_nonzero) >= 2:
            ...
            mds = MDS(n_components=2, metric="precomputed", random_state=42,
                      normalized_stress="auto", n_init=4, init="random")
