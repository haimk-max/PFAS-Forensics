...
### 3. מנוע ייחוס-מקור (forensic attribution)
- **שלושה צירי ראיה**: כימי (התאמת פרופיל-מקור), הידרולוגי (מעלה/מורד, דעיכה, הזדקנות), פליטה (ראיה עצמאית)
- **משפחות-הסעה**: מוקד · נחל · מוזנות-שאיבה · מתועל · החדרת-גדות · תהום — כל תחנה משויכת לנתיב, לא רק לרשימה
- **מדרגות-עננה לתהום** (עננה גאוסיאנית, k מוצהר) ו**סיווג קידוחי ניטור/הפקה** (אזור-לכידה מטשטש מיקום)
- **מבחן הצטרפות-עומס בצמתים**: ארבעה גלאי-מקטע לאיתור מקור נוסף לאורך גזע-זרימה
- **תקרת-הנחה**: אין דירוג מעל "מועמד משני" כל עוד ראיה נספרת נשענת על כיוון-זרימה מונח
- פלט תמיד עם **בעד / נגד / מה-יפריך**; ניסוח-ייחוס זהיר (מועמד מרכזי/משני — לעולם לא "המקור")

### 4. דוח-חקירה פר-תיק (generate_case_report.py)
- מסמך HTML עצמאי אחד לכל תיק: רקע, ממצאים לפי משפחות-הסעה (פרוזה + כרטיסי-נתונים), דיון, מסקנות בסולם-ודאות, פעולות נדרשות ונספחים
- **מפה אינטראקטיבית מוטמעת** (Leaflet מקומי): רקע תצ"א Sentinel-2 + הצללת-תבליט, רשת-ערוצים, מסלולי-נגר, נתיבים מוצהרים, צמתים חשודים; זום/הזזה, שכבות, פרטי-תחנה בלחיצה
- איורים: מודל-אתר קונספטואלי (CSM), מטריצת-דמיון, דעיכה והזדקנות, הרכבים יחסיים — כולם מסתננים לפי משפחה
- **ישות-תיק** (`regions/<תיק>/`): bbox, סמנטיקת-חתך, מקורות, לוח-טענות (claims) ותור-פעולות (actions) — כל שכבה עם provenance

### 5. דשבורד Streamlit אינטראקטיבי (app.py)
...
### 6. דוחות HTML עצמאיים (generate_report.py / generate_report_v2.py)
...
הדוחות משתמשים בשפת "מועמד מרכזי" / "מועמד משני" / "מקור אפשרי" — **לעולם לא "המקור"**.
